...
function preflight(list, env) {
  ...
  if (fatal.length) {
    throw new Error(`preflight: ${fatal.length} required credential(s) missing (see above). Set them in .env and re-run — nothing was built; refusing to produce a degraded result silently.`);
  }
...
export async function run(repoUrl, opts = {}) {
  ...
  const apiKey = getSecret(env, ['ANTHROPIC_API_KEY', 'CLAUDE_API_KEY']);
  ...
  // Preflight: name any missing credential for the stations in this slice + how to set it; refuse to
  // start on a fatal gap rather than build a degraded result. (The brain key is among the fatal checks.)
  preflight(list, env);
